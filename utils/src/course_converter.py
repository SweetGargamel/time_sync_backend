...
def _calculate_date(semester_start, week_num, weekday):
    # 将学期开始日期转换为datetime对象
    start_date = semester_start
    # 计算目标日期
    target_date = start_date + timedelta(days=(week_num-1)*7 + weekday)
...
def convert_to_events(courses_data: list, semester_start: str, username: str) -> dict:
    """
    将课程数据转换为事件格式
    
    Args:
        courses_data (list): 课程数据列表
        semester_start (str): 学期开始日期，格式为 'YYYY-MM-DD'
        username (str): 用户名
